		...
		<el-header height="100%"
			...
			<div class="page-tabs page-tabs-style-one">
				<el-dropdown trigger="contextmenu"
					...
					<div class="page-tab-item"
						...
						<div :class="{'item-active':item.isSelected}">
							...
							<div class="close-btn-container">
								<i class="close-btn iconfont icon-pinned"
					...
					<template #dropdown>
						<el-dropdown-menu>
							<el-dropdown-item v-for="menu in dropdownAffixOptions"
																:key="menu"
					...
					<el-dropdown trigger="contextmenu"
						...
						<div class="page-tab-item"
							...
							<div :class="{'item-active':item.isSelected}">
								...
								<span>{{item.meta.title}}</span>
								<div class="close-btn-container">
									<i class="close-btn iconfont icon-guanbi2"
						...
						<template #dropdown>
							<el-dropdown-menu>
								<el-dropdown-item v-for="menu in dropdownOptions"
																	:key="menu"
																	@click="OnTabDropdownClick(item, menu.label)">
									<i class="iconfont"
										 :class="menu.icon" />
...
const ishide = computed(() => {
	...
	if (props.isTopNav) {
		return !systemTheme.value.showPageTabs || (systemTheme.value.showPageTabs && systemTheme.value.layout !== 'horizontal');
	} else {
		return !systemTheme.value.showPageTabs || systemTheme.value.layout === 'horizontal' || systemTheme.value.layout === 'chapter'
	}
});
/** 页签控制 Start */
...
const OnTabDropdownClick = (tab, label) => {
	...
	switch (label) {
		...
		case '关闭其它':
			...
			storesUseKeepALiveNames.setCacheKeepAlive(pageTabs.value.filter(item => item.meta.keepAlive).map(item => item.name));
		...
		case '关闭左边':
			...
			for (let index = 0; index < pageTabs.value.length; index++) {
				...
				if (!pageTabs.value[index].meta.isAffix && !pageTabs.value[index].meta.mainPage) {
			...
			storesUseAppSettings.setPageTabs(pageTabs.value);
			storesUseKeepALiveNames.setCacheKeepAlive(pageTabs.value.filter(item => item.meta.keepAlive).map(item => item.name));
			if (tab.path !== route.path && pageTabs.value.filter(item => item.path === route.path).length === 0) {
				router.push(tab.path);
			}
			break;
		case '关闭右边':
			_delTabs = [];
			for (let index = pageTabs.value.length - 1; index < pageTabs.value.length; index--) {
				if (pageTabs.value[index].path === tab.path) break;
				if (!pageTabs.value[index].meta.isAffix && !pageTabs.value[index].meta.mainPage) {
					_delTabs.push(pageTabs.value[index].name);
					continue;
		...
		case '关闭所有':
			...
			storesUseAppSettings.setPageTabs(pageTabs.value);
			storesUseKeepALiveNames.setCacheKeepAlive(pageTabs.value.filter(item => item.meta.keepAlive).map(item => item.name));
			if (pageTabs.value.length !== 0 && pageTabs.value.filter(item => item.path === route.path).length === 0) {
...
const OnTabsChange = (toRoute) => {
	...
	pageTabs.value.forEach(item => { item.isSelected = item.path === toRoute.path; });
...
const OnInitTabList = () => {
	...
	GetRouteList(route.matched).forEach((item) => {
		if (item.meta && item.meta.isAffix && routeList.filter(i => i.path === item.path).length === 0) {
...
const InitDropTable = () => {
	if (dragTable.value) {
		Sortable.create(document.querySelector(`.tabs-drag-${props.isTopNav ? 'top' : 'nottop'}`), {
			animation: 500,
...
onBeforeRouteUpdate((to) => {
	if (pageTabs.value.filter((item) => to.path === item.path).length === 0) {
		pageTabs.value.forEach(item => { item.isSelected = false; });
		to.isSelected = true;
		pageTabs.value.push(to);
	} else {
		pageTabs.value.forEach(item => { item.isSelected = item.path === to.path; });
	}
	storesUseAppSettings.setPageTabs(pageTabs.value);
	if (to.meta.isAffix && to.meta.keepAlive && cachedViews.value.indexOf(to.name) === -1) {
